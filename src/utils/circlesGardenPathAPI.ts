...
const MAX_VALUE_FROM_PATH = '1000000000000000000000000000000000'
...
export const getPath = async (fromAddress: string, toAddress: string) => {
  const url = `${CIRCLES_GARDEN_API}transfers/`
  const body = {
    from: fromAddress,
    to: toAddress,
  ...
  try {
    ...
    const data = (await response.json()) as CirclesPathfinderResponse
    return data
  } catch (_e) {
    console.log({ _e })
    return undefined
...
export const transformPathToTransferThroughParams = (
  ...
  const tokenOwners = path.map((p) => p.tokenOwnerAddress)
